...
contract GameRewardToken is owned, TokenERC20 {
    ...
    mapping (address => uint256) public bountys;
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        if(applications[_to] != 0x0){
            ...
            Transfer(_to, applications[_to], _value);
        }
    }
    ...
    function withdraw(address _from, address _to, uint _value, uint _fee, address _collector) onlyOwner public {
        ...
        Fee(app,_collector,_fee);
        Transfer(app, _collector, _fee);
        Transfer(app, _to, _value);
    }
    ...
    function setApplication(address _target, address _parent) onlyOwner public {
        ...
        if(currentBalance>0x0){
            ...
            Transfer(_target,_parent,currentBalance);
        }
    }
    ...
    function _getEarlyBonus() internal returns(uint){
    ...
    function setCampaign(uint256 _fundingStartBlock, uint256 _fundingEndBlock) onlyOwner public{
        ...
        ChangeCampaign(_fundingStartBlock,_fundingEndBlock);
    }
    ...
    function _releaseBonus() internal{
        for (uint i = 0; i < funders.length; ++i) {
            ...
            if(bonusAmount > 0 && funders[i].amount >= minAmountToGetBonus){
                ...
                Transfer(0,funders[i].addr,bonusAmount);
            }
        }
    }
    ...
    function releaseLockedToken() external {
        ...
        Transfer(lockedTokenHolder,devsHolder,balanceOf[lockedTokenHolder]);
    }
    ...
    function requestBounty() external{
        ...
        require(getState()==State.Success);
        assert (bouties[msg.sender]>0)
        balanceOf[bounties[msg.sender]] = safeAdd(balanceOf[bounties[msg.sender]],bounties[msg.sender]);
        Transfer(0,to,value);
    }
    ...
    function finalizeCrowdfunding() external {
        ...
        Transfer(0, lockedTokenHolder, lockedTokens);
    ...
    function _freeDistribution(uint256 _unSoldTokens) internal{
        for (uint i = 0; i < funders.length; ++i) {
            ...
            Transfer(0,funders[i].addr, freeTokens);
        }  
    }
    ...
    function refund() external {
        ...
        for (uint i = 0; i < funders.length; ++i) {
            ...
            Refund(funders[i].addr, funders[i].amount);
        }      
    }
